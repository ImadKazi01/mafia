...
export function Game({
  ...
  const updatedPlayerInfo = gameState.players.find(
  ...
  const canAct =
    isNight &&
    !isSpectator &&
    (currentPlayer?.role === "mafia" || currentPlayer?.role === "doctor");
  const canVote =
    !isNight && !isSpectator && currentPlayer?.role !== "narrator";
    ...
    <>
      {isReconnecting && (
        <div className="fixed top-0 left-0 right-0 bg-yellow-900/90 p-4 text-center shadow-lg z-50">
          <p className="text-yellow-100 font-bold">Reconnecting to game...</p>
        </div>
      )}
      {isEliminated && (
                  ...
                  <div className="text-red-400 mt-2">
                    ...
                    {mafiaActions.length > 0 && (
                      <div className="mt-2">
                        ...
                        {mafiaActions
                          .filter(
                            (action) => action.mafiaName && action.targetName
                          )
                          .map((action, index) => (
                            <p key={index} className="text-sm">
                              {action.mafiaName} is targeting{" "}
                              {action.targetName}
                            </p>
                          ))}
                      </div>
              ...
              {gameState.message && currentPlayer?.role === "narrator" && (
                <div className="bg-gray-800 p-4 rounded-lg mb-6 text-center">
                  <p className="text-lg text-yellow-400">
                    Narrator Info: {gameState.message}
                  </p>
                </div>
              )}
                    ...
                    className={`
                      ...
                      ${
                        selectedPlayer === currentPlayer?.id
                          ? "ring-2 ring-green-400 bg-green-700"
                          : ""
                      }
                  ...
                  {targetablePlayers.map((player) => (
                    <button
                      ...
                      className={`
                        ...
                        ${
                          (canAct || canVote) && !isEliminated
                            ? selectedPlayer === player.id
                              ? "ring-2 ring-blue-500 bg-gray-600"
                              : "hover:bg-gray-600 hover:scale-[1.01] transition-all"
                            : "cursor-not-allowed"
                        }
                        relative overflow-hidden group
                      `}
                    >
                      <div className="flex items-center gap-3 relative z-10">
                        {getRoleIcon(
                          shouldShowRole(player) ? player.role : undefined
                        )}
                        <span
                          className={`
                      ...
                      {selectedPlayer === player.id && (
                        <span className="absolute inset-0 bg-blue-400/10 animate-pulse"></span>
                      )}
                    </button>
                  ))}
                </div>
